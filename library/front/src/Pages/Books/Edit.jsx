...
export default function Edit() {
    ...
    const submit = _ => {
        ...
        const authorIds = authorsDropdown.map(author => author.id);

        const errors = new Map();
        v.validate(inputs.title, 'title', errors, [v.required, v.string, [v.min, 3], [v.max, 100]]);
        v.validate(inputs.pages, 'pages', errors, [v.required, v.integer, [v.min, 3], [v.max, 10000]]);
        v.validate(inputs.genre, 'genre', errors, [v.required, v.string, [v.min, 3], [v.max, 100]]);
        v.validate(inputs.author_id, 'author_id', errors, [v.required, [v.inNumbers, authorIds]]);
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <div className="mb-3">
                            ...
                            <input type="text" className="form-control" style={{borderColor: e.has('title') ? 'crimson' : null }} id="title" value={inputs.title} onChange={handlerChange} />
                        </div>
                        <div className="mb-3">
                            <label htmlFor="pages" className="form-label">Pages</label>
                            <input type="text" className="form-control" id="pages" style={{borderColor: e.has('pages') ? 'crimson' : null }} value={inputs.pages} onChange={handlerChange} />
                        </div>
                        <div className="mb-3">
                            <label htmlFor="genre" className="form-label">Genre</label>
                            <input type="text" className="form-control" style={{borderColor: e.has('genre') ? 'crimson' : null }} id="genre" value={inputs.genre} onChange={handlerChange} />
                        </div>
                        {
                            authorsDropdown &&
                            <div className="mb-3">
                                <label htmlFor="author_id" className="form-label">Author</label>
                                <select className="form-select" style={{borderColor: e.has('author_id') ? 'crimson' : null }} id="author_id" value={inputs.author_id} onChange={handlerChange}>
